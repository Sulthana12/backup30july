...
namespace MileDALibrary.DataRepository
{
    public class BankDetailsRepository : IBankDetailsRepository
    {
        public async Task<BankDetails> GetBankDetails(string ifscCode)
        {
            ...
            {
                using (var response = await httpClient.GetAsync($"https://ifsc.razorpay.com/{ifscCode}"))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
                    bankDetails = JsonConvert.DeserializeObject<BankDetails>(apiResponse);
                    if (bankDetails != null)
                    {
                        return bankDetails;
                    }
            ...
            //    {
            //        string apiResponse = await response.Content.ReadAsStringAsync();
            //        BankDetails bank = JsonConvert.DeserializeObject<BankDetails>(apiResponse);
